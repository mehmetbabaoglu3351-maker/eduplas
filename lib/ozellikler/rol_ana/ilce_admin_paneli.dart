...
class IlceAdminPaneli extends StatelessWidget {
  const IlceAdminPaneli({super.key});

  Future<String> _rolGetir() async {
    final uid = FirebaseAuth.instance.currentUser!.uid;
    final u = await FirebaseFirestore.instance.collection('users').doc(uid).get();
    return (u.data() ?? const {})['role']?.toString() ?? 'İlçe Admin';
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('İlçe Admin Paneli')),
      body: Center(
          ...
          child: Card(
            child: Padding(
              ...
              child: FutureBuilder<String>(
                future: _rolGetir(),
                builder: (ctx, snap) {
                  final rol = snap.data ?? 'İlçe Admin';
                  return Column(
