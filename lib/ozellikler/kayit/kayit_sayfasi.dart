...
class _KayitSayfasiState extends State<KayitSayfasi> {
  ...
  // Versiyon etiketleri (sürüm değiştirirsen yalnız burayı güncelle)
  ...
  // Konum (hiyerarşi ve/veya cihaz)
  SeciliYer? _adres; // iç isim korundu; UI'da "Adres" geçmez
  LocationFix? _fix;

  // Diğer durumlar
  bool _islem = false;
  String? _hata;
  bool _bilgiOnayi = false; // "Bilgilerim doğru..." onayı (uygulama içi)
  ...
  // Basit slug (TR karakter normalize + trim + lower + yalnız a-z0-9._-)
  ...
  // TR odaklı doğrulama: sadece rakamlar ve 10 hane
  bool _gecerliTelefonZorunlu(String v) {
    final t = v.replaceAll(RegExp(r'[^0-9]'), '');
    if (t.isEmpty) return false; // telefon zorunlu
    return t.length == 10; // örn: 5xx xxx xx xx
    // Not: Firestore'a +90 eklemeden 10 hane gömüyoruz; SMS akışında prefix eklenebilir.
  }
  ...
  bool _isBasAdmin(String role) {
    ...
    return r.contains('baş admin') || r.contains('bas admin');
  }

  bool get _konumVar => _adres != null || _fix != null;

  // Tekil location objesi: hem seçilen hiyerarşi hem cihaz fix'i
  ...
  Future<void> _kayit() async {
    ...
    try {
      ...
      // Kayıt ve kullanıcı referansları
      ...
      if (_isBasAdmin(requestedRole)) {
        ...
        await userRef.set(baseUser, SetOptions(merge: true));
        await regRef.set({
          ...baseReg,
          'status': 'pending',
          'approverUid': null,
      ...
      // --- MOCK OnayServisi besleme (IN-MEMORY) ---
final basvuru = Basvuru(
  uid: uid,
  takmaAd: temizNick,
  telefon: tel,
  requestedRole: requestedRole,
  location: location, // tekil kaynak
  okul: okul,
  status: _isBasAdmin(requestedRole) ? 'approved' : 'pending',
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      validator: (v) => (v != null && _gecerliTelefonZorunlu(v))
                          ? null
                          : 'Telefonu 5XX ile başlayan 10 hane girin.',
                    ),
                    gap,
                    TextFormField(
                      controller: _sifre,
                    ...
                    // --- Konum (Adres metin alanları yok) ---
                    const _GrupBaslik('Konum'),
                    KonumAdresSecici(
                      konumButonu: true, // "Cihazdan Al" butonu
                      onChanged: (secim, fix) {
                        setState(() {
                          _adres = secim;
                          _fix = fix;
                      ...
                      children: _roller.map((rol) {
                        ...
                        return ChoiceChip(
                          ...
                          onSelected: (_) => setState(() {
                            _seciliRol = rol;
                            _bilgiOnayi = false; // rol değişti, onayı sıfırla
                          }),
                        );
                      }).toList(),
                    ),
                    ...
                    CheckboxListTile(
                      ...
                      title: const Text(
                        'Açık Rıza Metnini onaylıyorum (kişisel verilerimin işlenmesi ve yurt dışına aktarımı).',
                      ),
                      controlAffinity: ListTileControlAffinity.leading,
                    ),
                    ...
                    const SizedBox(height: 8),
                    const Text(
                      'Not: Telefon doğrulama makettir; kod ekranda gösterilir.',
